...
<div class="list-icons position-relative">
    <div class="dropdown">
        ...
        <div class="dropdown-menu dropdown-menu-end dropdown-menu-right shadow border-0 rounded-3 p-2" style="min-width: 140px; z-index: 1060;">
           @if ($subModule && !$subModule->deleted_at)
                @if (!empty($pageActions['action']))
                    @foreach($pageActions['action'] as $action)
                        ...
                        @endphp
                        @if ($actType == 'destroy')
                            <form id="action-form-submodule-{{ $actType }}-{{ $subModule->id }}" action="{{ Route::has($actRoute) ? route($actRoute, [$subModule->id]) : route('admin.menu-settings.sub-modules.destroy', $subModule->id) }}" method="POST" style="display:none;">
                                @csrf
                                @method('DELETE')
                            </form>
                            <a href="javascript:confirm('action-form-submodule-{{ $actType }}-{{ $subModule->id }}')" class="dropdown-item text-danger fw-bold rounded-2 py-2">
                                <i class="{{ $actIcon }} me-2"></i>{{ $actName }}
                            </a>
                        @elseif ($actType == 'edit_modal')
                            <button type="button" class="dropdown-item text-dark fw-semibold rounded-2 py-2" data-bs-toggle="modal" data-bs-target="#editSubModuleModal{{ $subModule->id }}">
                                <i class="{{ $actIcon }} me-2 text-secondary"></i>{{ $actName }}
                            </button>
                        @else
                            <a href="{{ Route::has($actRoute) ? route($actRoute, [$subModule->id]) : '#' }}" class="dropdown-item text-dark fw-semibold rounded-2 py-2">
                                <i class="{{ $actIcon }} me-2 text-secondary"></i>{{ $actName }}
                            </a>
                        @endif
                    @endforeach
                @else
                    ...
                    <form id="delete-submodule-{{ $subModule->id }}" action="{{ route('admin.menu-settings.sub-modules.destroy', $subModule->id) }}" method="POST" style="display:none;">
                        @csrf
                        @method('DELETE')
                    </form>
                    <a href="javascript:confirm('delete-submodule-{{ $subModule->id }}')" class="dropdown-item text-danger fw-bold rounded-2 py-2">
