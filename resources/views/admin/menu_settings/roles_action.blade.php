@php
    $module = $role ?? $module ?? $model ?? $item ?? $data ?? $row ?? null;
           ...
           @if ($module)
                @if (!empty($pageActions['action']))
                    @foreach($pageActions['action'] as $action)
                        ...
                        @endphp
                        @if ($actType == 'destroy')
                            <form id="action-form-role-{{ $actType }}-{{ $module->id }}" action="{{ Route::has($actRoute) ? route($actRoute, [$module->id]) : route('admin.roles.destroy', $module->id) }}" method="POST" style="display:none;">
                                @csrf
                                @method('DELETE')
                            </form>
                            <a href="javascript:confirm('action-form-role-{{ $actType }}-{{ $module->id }}')" class="dropdown-item text-danger fw-bold rounded-2 py-2">
                                <i class="{{ $actIcon }} me-2"></i>{{ $actName }}
                            </a>
                        @elseif ($actType == 'edit_modal')
                            <button type="button" class="dropdown-item text-dark fw-semibold rounded-2 py-2" data-bs-toggle="modal" data-bs-target="#editRoleModal{{ $module->id }}">
                                <i class="{{ $actIcon }} me-2 text-secondary"></i>{{ $actName }}
                            </button>
                        @else
                            <a href="{{ Route::has($actRoute) ? route($actRoute, [$module->id]) : route('admin.roles.edit', $module->id) }}" class="dropdown-item text-dark fw-semibold rounded-2 py-2">
                                <i class="{{ $actIcon }} me-2 text-secondary"></i>{{ $actName }}
                            </a>
                        @endif
                    @endforeach
                @else
                    <a href="{{ route('admin.roles.edit', $module->id) }}" class="dropdown-item text-dark fw-semibold rounded-2 py-2">
                        <i class="fas fa-pen-nib text-secondary me-2"></i>{{ __('app.edit') }}
                    </a>
                    <form id="delete-role-{{ $module->id }}" action="{{ route('admin.roles.destroy', $module->id) }}" method="POST" style="display:none;">
                        @csrf
                        @method('DELETE')
                    </form>
                    <a href="javascript:confirm('delete-role-{{ $module->id }}')" class="dropdown-item text-danger fw-bold rounded-2 py-2">
