@php
    $module = $page ?? $module ?? $model ?? $item ?? $data ?? $row ?? null;
           ...
           @if ($module)
                @if (!empty($pageActions['action']))
                    @foreach($pageActions['action'] as $action)
                        ...
                        @endphp
                        @if ($actType == 'destroy')
                            <form id="action-form-page-{{ $actType }}-{{ $module->id }}" action="{{ Route::has($actRoute) ? route($actRoute, [$module->id]) : route('admin.menu-settings.pages.destroy', $module->id) }}" method="POST" style="display:none;">
                                @csrf
                                @method('DELETE')
                            </form>
                            <a href="javascript:confirm('action-form-page-{{ $actType }}-{{ $module->id }}')" class="dropdown-item text-danger fw-bold rounded-2 py-2">
                                <i class="{{ $actIcon }} me-2"></i>{{ $actName }}
                            </a>
                        @elseif ($actType == 'edit_modal')
                            <button type="button" class="dropdown-item text-dark fw-semibold rounded-2 py-2" data-bs-toggle="modal" data-bs-target="#editPageModal{{ $module->id }}">
                ...
                @else
                    ...
                    <form id="delete-page-{{ $module->id }}" action="{{ route('admin.menu-settings.pages.destroy', $module->id) }}" method="POST" style="display:none;">
                        @csrf
                        @method('DELETE')
                    </form>
                    <a href="javascript:confirm('delete-page-{{ $module->id }}')" class="dropdown-item text-danger fw-bold rounded-2 py-2">
