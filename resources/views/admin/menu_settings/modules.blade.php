...
@section('title', __('Modules Setup'))
...
<div class="card border-0 shadow-sm rounded-4 mb-4">
    <div class="card-body p-3">
        <ul class="nav nav-pills nav-fill gap-2">
            <li class="nav-item">
                <a class="nav-link active fw-bold text-white shadow-sm" style="background-color: #1b2559;" href="{{ route('admin.menu-settings.modules.index') }}">
...
<div class="dreams-card p-0 overflow-hidden border-0 shadow-sm">
    ...
    <div class="p-4 bg-white border-bottom d-flex flex-wrap align-items-center justify-content-between gap-3">
        <div>
            <h4 class="dreams-card-title mb-1"><i class="fas fa-cubes me-2" style="color: #1877f2;"></i> {{ __('Modules Setup') }}</h4>
            <p class="text-muted small mb-0">Configure system modules, display labels, icons, and order.</p>
        </div>
        <div class="d-flex flex-wrap align-items-center gap-2">
            <a href="{{ route('admin.menu-settings.export-json') }}" class="btn btn-outline-success rounded-pill px-3">
                <i class="fas fa-file-export me-1"></i> {{ __('Export JSON Structure') }}
            </a>
            <button type="button" class="btn btn-outline-info rounded-pill px-3" data-bs-toggle="modal" data-bs-target="#importJsonModal">
                <i class="fas fa-file-import me-1"></i> {{ __('Import JSON Package') }}
            </button>
            <a href="{{ route('admin.menu-settings.modules.download-template') }}" class="btn btn-outline-secondary rounded-pill px-3">
                <i class="fas fa-download me-1"></i> {{ __('Template') }}
            </a>
            <button type="button" class="btn btn-outline-primary rounded-pill px-3" data-bs-toggle="modal" data-bs-target="#importModuleModal">
                <i class="fas fa-file-upload me-1"></i> {{ __('Import CSV') }}
            </button>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow">
            <div class="modal-header border-bottom bg-light py-3 px-4" style="border-top: 3px solid #1b2559 !important;">
                <h5 class="modal-title fw-bold" style="color: #1877f2;"><i class="fas fa-folder-plus me-2"></i>{{ __('Add Module') }}</h5>
            ...
            <form action="{{ route('admin.menu-settings.modules.store') }}" method="POST">
                ...
                <div class="modal-body p-4 text-start">
                    <div class="mb-3">
                        <label class="form-label fw-bold">Module Name (EN) <span class="text-danger">*</span></label>
                        <input type="text" name="name" class="form-control" placeholder="Please input module name (EN)" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold">Module Name (KH)</label>
                        <input type="text" name="name_kh" class="form-control" placeholder="Please input module name (KH)">
                    </div>
                    <div class="mb-3">
...
<!-- Import Module Modal -->
<div class="modal fade" id="importModuleModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow">
            <div class="modal-header border-bottom bg-light py-3 px-4">
                <h5 class="modal-title fw-bold"><i class="fas fa-file-upload me-2 text-primary"></i>{{ __('Import Modules') }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('admin.menu-settings.modules.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
...
<!-- Import System JSON Package Modal -->
<div class="modal fade" id="importJsonModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow">
            <div class="modal-header border-bottom bg-light py-3 px-4">
                <h5 class="modal-title fw-bold"><i class="fas fa-file-import me-2 text-info"></i>{{ __('Import System JSON Package') }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('admin.menu-settings.import-json') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label class="form-label fw-bold">Structure JSON File <span class="text-danger">*</span></label>
                        <input type="file" name="file" class="form-control" accept=".json" required>
                    </div>
                </div>
                <div class="modal-footer border-top bg-light">
                    <button type="button" class="btn btn-secondary px-4" data-bs-dismiss="modal">{{ __('Cancel') }}</button>
                    <button type="submit" class="btn btn-info text-white px-4">{{ __('Sync Structure') }}</button>
                </div>
            </form>
        </div>
    </div>
...
<!-- Edit Module Modal {{ $m->id }} -->
<div class="modal fade" id="editModuleModal{{ $m->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow">
            <div class="modal-header border-bottom bg-light py-3 px-4" style="border-top: 3px solid #1b2559 !important;">
                <h5 class="modal-title fw-bold" style="color: #1877f2;"><i class="fas fa-edit me-2"></i>{{ __('Edit Module') }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('admin.menu-settings.modules.update', $m->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body p-4 text-start">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold">Module Name (KH)</label>
                        <input type="text" name="name_kh" class="form-control" value="{{ $m->name_kh }}">
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold">Icon Class (FontAwesome) <span class="text-danger">*</span></label>
                        <input type="text" name="icon" class="form-control" value="{{ $m->icon }}" required>
                    </div>
                    <div class="row g-3">
                        <div class="col-6">
                            <label class="form-label fw-bold">Sort Order</label>
                            <input type="number" name="sort_order" class="form-control" value="{{ $m->sort_order }}">
                        </div>
                        <div class="col-6">
                            <label class="form-label fw-bold">Status <span class="text-danger">*</span></label>
                            <select name="status" class="form-select">
