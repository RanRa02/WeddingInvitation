...
@section('title', __('Form Edit - User'))
...
<div class="card border border-light-subtle shadow-sm rounded-3 overflow-hidden mb-4" style="border-top: 3px solid #1b2559 !important;">
    <div class="card-header bg-white py-3 px-4 d-flex align-items-center justify-content-between border-bottom border-light-subtle">
        ...
        <a data-bs-toggle="collapse" href="#userEditFormCard" role="button" aria-expanded="true" aria-controls="userEditFormCard" class="text-primary fs-6 text-decoration-none">
            <i class="fas fa-chevron-down"></i>
        </a>
    </div>
    <div class="collapse show" id="userEditFormCard">
            ...
            <div class="col-12 col-md-9 col-lg-7 mx-auto">
                <form action="{{ route('admin.users.update', $user) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $user->name) }}" placeholder="Please input full name" required>
                        @error('name') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-bold text-dark mb-1">{{ __('Email Address') }} <span class="text-danger">*</span></label>
                        <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email', $user->email) }}" placeholder="Please input email address" required>
                        @error('email') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-bold text-dark mb-1">{{ __('New Password (Leave blank to keep current)') }}</label>
                        <input type="password" name="password" class="form-control @error('password') is-invalid @enderror" placeholder="Please input new password">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" class="form-select" required>
                            <option value="">Please Select</option>
                            <option value="active" {{ old('status', $user->status) === 'active' ? 'selected' : '' }}>Active (សកម្ម)</option>
                            <option value="inactive" {{ old('status', $user->status) === 'inactive' ? 'selected' : '' }}>Inactive (ផ្អាក)</option>
                        </select>
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-bold text-dark mb-1">{{ __('Max Guest Limit') }} <span class="text-danger">*</span></label>
                        <input type="number" name="guest_limit" min="1" max="100000" class="form-control" value="{{ old('guest_limit', $user->guest_limit ?? 100) }}" placeholder="Please input max guest limit" required>
                    </div>
