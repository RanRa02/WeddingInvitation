@php
    $module = $user ?? $module ?? $model ?? $item ?? $data ?? $row ?? null;
                    ...
                    @foreach($pageActions['action'] as $action)
                        ...
                        @if ($actType == 'destroy')
                            @if($module->id !== auth()->id())
                                <form id="action-form-user-{{ $actType }}-{{ $module->id }}" action="{{ Route::has($actRoute) ? route($actRoute, [$module->id]) : route('admin.users.destroy', $module->id) }}" method="POST" style="display:none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
                                <a href="javascript:confirm('action-form-user-{{ $actType }}-{{ $module->id }}')" class="dropdown-item text-danger fw-bold rounded-2 py-2">
                                    <i class="{{ $actIcon }} me-2"></i>{{ $actName }}
                                </a>
                            @endif
                        @elseif ($actType == 'edit_modal')
                            <button type="button" class="dropdown-item text-dark fw-semibold rounded-2 py-2" data-bs-toggle="modal" data-bs-target="#editUserModal{{ $module->id }}">
                                <i class="{{ $actIcon }} me-2 text-secondary"></i>{{ $actName }}
                            </button>
                        @else
                            <a href="{{ Route::has($actRoute) ? route($actRoute, [$module->id]) : route('admin.users.edit', $module->id) }}" class="dropdown-item text-dark fw-semibold rounded-2 py-2">
                                <i class="{{ $actIcon }} me-2 text-secondary"></i>{{ $actName }}
                            </a>
                        @endif
                    @endforeach
                @else
                    <a href="{{ route('admin.users.edit', $module->id) }}" class="dropdown-item text-dark fw-semibold rounded-2 py-2">
                        <i class="fas fa-pen-nib text-secondary me-2"></i>{{ __('app.edit') }}
                    </a>
                    @if ($module->id !== auth()->id())
                        <form id="delete-user-{{ $module->id }}" action="{{ route('admin.users.destroy', $module->id) }}" method="POST" style="display:none;">
                            @csrf
                            @method('DELETE')
                        </form>
                        <a href="javascript:confirm('delete-user-{{ $module->id }}')" class="dropdown-item text-danger fw-bold rounded-2 py-2">
