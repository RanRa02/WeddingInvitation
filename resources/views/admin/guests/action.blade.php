           ...
           @if ($module)
                @if (!empty($pageActions['action']))
                    @foreach($pageActions['action'] as $action)
                        ...
                        @endphp
                        @if ($actType == 'destroy')
                            <form id="action-form-guest-{{ $actType }}-{{ $module->id }}" action="{{ Route::has($actRoute) ? route($actRoute, [$module->id]) : route('admin.guests.destroy', $module->id) }}" method="POST" style="display:none;">
                                @csrf
                                @method('DELETE')
                            </form>
                            <a href="javascript:confirm('action-form-guest-{{ $actType }}-{{ $module->id }}')" class="dropdown-item text-danger fw-bold rounded-2 py-2">
                        ...
                        @else
                            <a href="{{ Route::has($actRoute) ? route($actRoute, [$module->id]) : route('admin.guests.edit', $module->id) }}" class="dropdown-item text-dark fw-semibold rounded-2 py-2">
                                <i class="{{ $actIcon }} me-2 text-secondary"></i>{{ $actName }}
                            </a>
                        @endif
                    @endforeach
                @else
                    <a href="{{ route('admin.guests.edit', $module->id) }}" class="dropdown-item text-dark fw-semibold rounded-2 py-2">
                        <i class="fas fa-pen-nib text-secondary me-2"></i>{{ __('app.edit') }}
                    </a>
                    <form id="delete-guest-{{ $module->id }}" action="{{ route('admin.guests.destroy', $module->id) }}" method="POST" style="display:none;">
                        @csrf
                        @method('DELETE')
                    </form>
                    <a href="javascript:confirm('delete-guest-{{ $module->id }}')" class="dropdown-item text-danger fw-bold rounded-2 py-2">
