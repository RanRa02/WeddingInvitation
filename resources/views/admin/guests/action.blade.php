                ...
                @isset($pageActions['action'])
                    @foreach($pageActions['action'] as $action)
                        @if ($action['action_type'] == 'destroy')
                            <form id="action-form-guest-{{ $action['action_type'] }}-{{ $module->id }}" action="{{ Route::has($action['action_route']) ? route($action['action_route'], [$module->id]) : route('admin.guests.destroy', $module->id) }}" method="POST" style="display:none;">
                                @csrf
                                @method('DELETE')
                            </form>
                            <a href="javascript:confirm('{{ Route::has($action['action_route']) ? route($action['action_route'], [$module->id]) : route('admin.guests.destroy', $module->id) }}', 'action-form-guest-{{ $action['action_type'] }}-{{ $module->id }}')" class="dropdown-item text-danger fw-bold rounded-2 py-2">
                                <i class="{{ $action['action_icon'] }} me-2"></i>{{ $action['action_name'. $locale] ?? $action['action_name'] }}
                            </a>
                        @else
                            <a href="{{ Route::has($action['action_route']) ? route($action['action_route'], [$module->id]) : route('admin.guests.edit', $module->id) }}" class="dropdown-item text-dark fw-semibold rounded-2 py-2">
