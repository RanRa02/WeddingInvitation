@php
    $module = $plan ?? $module ?? $model ?? $item ?? $data ?? $row ?? null;
           ...
           @if ($module)
                @if (!empty($pageActions['action']))
                    @foreach($pageActions['action'] as $action)
                        ...
                        @endphp
                        @if ($actType == 'destroy')
                            <form id="action-form-plan-{{ $actType }}-{{ $module->id }}" action="{{ Route::has($actRoute) ? route($actRoute, [$module->id]) : route('admin.plans.destroy', $module->id) }}" method="POST" style="display:none;">
                                @csrf
                                @method('DELETE')
                            </form>
                            <a href="javascript:confirm('action-form-plan-{{ $actType }}-{{ $module->id }}')" class="dropdown-item text-danger fw-bold rounded-2 py-2">
                                <i class="{{ $actIcon }} me-2"></i>{{ $actName }}
                            </a>
                        @elseif ($actType == 'edit_modal')
                            <button type="button" class="dropdown-item text-dark fw-semibold rounded-2 py-2" data-bs-toggle="modal" data-bs-target="#editPlanModal{{ $module->id }}">
                ...
                @else
                    ...
                    <form id="delete-plan-{{ $module->id }}" action="{{ route('admin.plans.destroy', $module->id) }}" method="POST" style="display:none;">
                        @csrf
                        @method('DELETE')
                    </form>
                    <a href="javascript:confirm('delete-plan-{{ $module->id }}')" class="dropdown-item text-danger fw-bold rounded-2 py-2">
